...
const ownerid = ('336614900329611264')
...
Rocket.on('ready', () => { //playing
    Rocket.user.setGame(`x!help | Servers : ${Rocket.guilds.size}	`,'https://www.twitch.tv/faresgameryt');
...
const x5bz4 = [ //mariam
 ...
 if (message.content.startsWith('x!mariam')) {
...
Rocket.on('message', message => {//help msg
  ...
  By : XivoBot TeaM | __**شكرا لكم لاستعمال البوت**__
...
Rocket.on('message', message => {//unmute
    if (message.content.startsWith('x!unmute')) {
...
Rocket.on('message', message => {//mute
    if (message.content.startsWith('x!mute')) {
...
var currentTime = new Date(),
 ...
 Rocket.on('message', message => {//cuttweet
   if (message.content.startsWith("x!cuttweet")) {
                if(!message.channel.guild) return message.reply('** This command only for servers**');
...
Rocket.on('message', function(message) {//roll
    ...
    Rocket.on("message", message => {    //serv-av
        if(!message.channel.guild) return;
 if(message.author.bot) return;
    if(message.content === "x!serveravatar"){
        const embed = new Discord.RichEmbed()
...
if (message.content.startsWith('x!sarahah')) {
...
Rocket.on('message', msg => {
    ...
    Rocket.on('message', msg => {
        ...
        function commandIs(str, msg){
            return msg.content.toLowerCase().startsWith('x!' + str);
        }
